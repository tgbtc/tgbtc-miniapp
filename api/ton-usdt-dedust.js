export default async function handler(req, res) {
  try {
    const CHAIN = "ton";
    const PAIR = process.env.DEDUST_PAIR_ADDRESS; // EQ...

    if (!PAIR) {
      return res.status(500).json({ ok: false, error: "DEDUST_PAIR_ADDRESS missing" });
    }
    ...
    return res.json({ ok: true, dex: "dedust", price: ton_usdt, pair: PAIR });
  } catch (e) {
